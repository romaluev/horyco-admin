...
export default function BranchOverridesPage(): JSX.Element {
  ...
  const filteredOverrides = useMemo(() => {
    return overrides.filter((override) => {
      const isBranchMatch =
        selectedBranch === 'all' ||
        override.branchId === Number(selectedBranch);
  ...
  const stats = useMemo(() => {
    return {
      ...
      customAvailability: overrides.filter((o) => o.overrideAvailability !== null).length
    };
  }, [overrides]);
    ...
    <PageContainer>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.customAvailability}</div>
          </CardContent>
        </Card>
      </div>
            ...
            <Table>
              ...
              <TableBody>
                {filteredOverrides.map((override) => {
                  ...
                  return (
                    <TableRow key={`${override.productId}-${override.branchId}`}>
                      <TableCell className="font-medium">
                        {override.productName || `Продукт #${override.productId}`}
                      </TableCell>
                      <TableCell>
                        {override.branchName}
                      </TableCell>
                      <TableCell>
                        {override.overridePrice !== null ? (
                          <div className="flex items-center gap-2">
                            ...
                            {basePrice && override.overridePrice !== basePrice && (
                      ...
                      <TableCell>
                        {override.overrideAvailability !== null ? (
                          <Badge
                            variant={
                              override.overrideAvailability ? 'default' : 'secondary'
                            }
                          >
                            {override.overrideAvailability ? 'Доступен' : 'Недоступен'}
                          </Badge>
                        ) : (
