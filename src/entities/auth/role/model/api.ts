...
export const roleApi = {
  ...
  getAllRoles: async (
  ...
  ): Promise<IRole[] | IPaginatedResponse<IRole>> => {
    ...
    const response = await api.get<
      ApiResponse<IRole[] | IPaginatedResponse<IRole>>
    >('/admin/staff/roles', { params })
  ...
  getRoleById: async (id: number): Promise<IRole> => {
    const response = await api.get<ApiResponse<IRole>>(
  ...
  getAllPermissions: async (): Promise<IPermission[]> => {
    const response = await api.get<ApiResponse<IPermission[]>>(
  ...
  getSystemPermissions: async (): Promise<IPermission[]> => {
    const response = await api.get<ApiResponse<IPermission[]>>(
      '/admin/staff/permissions/system'
    )
    return response.data.data
  },
