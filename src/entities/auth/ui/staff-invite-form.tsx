...
import type { VerifyStaffInviteResponse } from '@/entities/auth/model/types'
...
const StaffInviteForm = () => {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      ...
      try {
        const response = await authApi.verifyStaffInvite({ token })
  ...
  const onSubmit = async (data: PasswordFormValues): Promise<void> => {
    ...
    try {
      ...
      const response = await authApi.completeStaffInvite({
        token,
      ...
      if (response.success) {
        ...
        // Redirect to dashboard for staff
