import { BaseError, BaseLoading, Checkbox, Label, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/shared/ui';
...
export const EmployeeFormBranches = ({ form }: EmployeeFormBranchesProps) => {
  ...
  const availableBranches = branchList.filter((branch) => selectedBranchIds.includes(branch.id));
    ...
    <div className='space-y-6'>
      ...
      <div className='space-y-4'>
        {branchList.map((branch) => (
          <div key={branch.id} className='flex items-start gap-4 rounded-lg border p-4'>
            <Checkbox
              id={`branch-${branch.id}`}
      ...
      {errors.branchIds && <p className='text-destructive text-sm'>{errors.branchIds.message}</p>}
      ...
      {selectedBranchIds.length > 0 && (
        <div className='space-y-2'>
          ...
          <Select
            ...
            onValueChange={(value) => setValue('activeBranchId', parseInt(value))}
          >
          ...
          {errors.activeBranchId && (
            <p className='text-destructive text-sm'>{errors.activeBranchId.message}</p>
          )}
        </div>
      )}
    </div>
