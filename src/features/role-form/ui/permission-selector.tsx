...
export const PermissionSelector = ({ form }: PermissionSelectorProps) => {
  ...
  const { data: permissionsGrouped, isLoading, isError } = useGetPermissionsGrouped();
  ...
  const selectedCount = (permissions: { id: number }[]): number => {
    return permissions.filter((p) => selectedPermissionIds.includes(p.id)).length;
  };
  ...
  return (
    <div className='space-y-4'>
      ...
      {errors.permissionIds && (
        <p className='text-destructive text-sm'>{errors.permissionIds.message}</p>
      )}
    </div>
  );
};
